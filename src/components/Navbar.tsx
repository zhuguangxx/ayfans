      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center space-x-3">
            ...
            <span className="font-bold text-lg md:text-xl hidden sm:block">安阳市球迷协会</span>
          </Link>

          {/* Desktop Menu */}
          <nav className="hidden md:flex items-center space-x-6">
            <Link to="/about" className="hover:text-red-200 transition">关于协会</Link>
            <Link to="/news" className="hover:text-red-200 transition">协会新闻</Link>
            <Link to="/contact" className="hover:text-red-200 transition">联系我们</Link>
            {user ? (
              <div className="flex items-center space-x-4">
                <Link to="/member" className="hover:text-red-200 transition">会员中心</Link>
                <button 
                  onClick={handleLogout}
                  className="bg-red-800 hover:bg-red-900 px-4 py-2 rounded-lg transition"
                >
                  退出
                </button>
              </div>
              ...
              <Link 
                to="/login" 
                className="bg-white text-red-600 hover:bg-red-100 px-4 py-2 rounded-lg transition font-medium"
              >
                会员登录
              </Link>
            )}
